class HrAbsence < ActiveRecord::Base
  ...
  has_many :audits, -> { order(:created_at) }, class_name: 'HrAbsenceAudit', dependent: :destroy
  ...
  def region_code
    @region_code ||= HrUserSetting.for(user).effective_region_code if user
  end
  ...
  def self.kind_label(kind)
    case kind
    when KIND_VACATION   then I18n.t(:label_hr_vacation)
    when KIND_SICKNESS   then I18n.t(:label_hr_sickness)
    when KIND_OFFSITE    then I18n.t(:label_hr_offsite)
    when KIND_SCHOOL     then I18n.t(:label_hr_school)
    when KIND_BLOCK      then I18n.t(:label_hr_block)
    when KIND_HOMEOFFICE then I18n.t(:label_hr_homeoffice)
    when KIND_CARE       then I18n.t(:label_hr_care)
    when KIND_WORKDAY    then I18n.t(:label_hr_workday)
  ...
  def self.recurrence_label(kind)
    case kind
    when RECURRENCE_WEEKLY     then I18n.t(:label_hr_recurrence_weekly)
    when RECURRENCE_BIWEEKLY   then I18n.t(:label_hr_recurrence_biweekly)
    when RECURRENCE_4_WEEKLY   then I18n.t(:label_hr_recurrence_four_weekly)
    when RECURRENCE_MONTHLY    then I18n.t(:label_hr_recurrence_monthly)
    when RECURRENCE_QUARTERLY  then I18n.t(:label_hr_recurrence_quarterly)
    when RECURRENCE_HALFYEARLY then I18n.t(:label_hr_recurrence_half_yearly)
    when RECURRENCE_YEARLY     then I18n.t(:label_hr_recurrence_yearly)
    else I18n.t(:label_hr_recurrence_none)
    end
  end
  ...
  def self.status_label(status)
    case status
    when STATUS_REQUESTED then I18n.t(:label_hr_absence_status_requested)
    when STATUS_APPROVED  then I18n.t(:label_hr_absence_status_approved)
    when STATUS_REJECTED  then I18n.t(:label_hr_absence_status_rejected)
    else status.to_s.humanize
    end
  end

  def log_audit!(actor, action, from_status: nil, to_status: nil, notes: nil)
    HrAbsenceAudit.create!(
      hr_absence_id: id,
      actor_id: actor.id,
      action: action,
  ...
  def approve_by!(actor, notes: nil)
    ...
    transaction do
      ...
      log_audit!(actor, HrAbsenceAudit::ACTION_APPROVED, from_status: prior, to_status: STATUS_APPROVED, notes: notes)
    end
  end
  ...
  def reject_by!(actor, notes: nil)
    ...
    transaction do
      ...
      log_audit!(actor, HrAbsenceAudit::ACTION_REJECTED, from_status: prior, to_status: STATUS_REJECTED, notes: notes)
    end
  end
  ...
  def self.vacation_working_days_used(user_id, year = Date.current.year)
    ...
    region = HrUserSetting.for(User.find(user_id)).effective_region_code rescue nil
  ...
  def time_range_consistent
    ...
    if start_time.blank? || end_time.blank?
      errors.add(:base, :hr_absence_time_range_incomplete) and return
    end
    s = parse_hhmm(start_time)
    e = parse_hhmm(end_time)
    if s.nil? || e.nil?
      errors.add(:base, :hr_absence_time_range_invalid) and return
    end
    errors.add(:base, :hr_absence_time_range_order) if e <= s
    errors.add(:base, :hr_absence_time_range_single_day) if starts_on && ends_on && starts_on != ends_on
  end
end
