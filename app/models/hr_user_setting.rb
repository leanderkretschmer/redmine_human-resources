class HrUserSetting < ActiveRecord::Base
  ...
  belongs_to :user
  belongs_to :hr_employment_type, optional: true
  ...
  def base_daily_target_minutes(on_date: nil)
    ...
    if on_date && allows_monthly_plan?
      plan_daily = HrMonthlyPlan.for_user(user).for_period(on_date.year, on_date.month).first
  ...
  def partial_absence_minutes_on(date)
    ...
    HrAbsence.for_user(user).active
             .where(starts_on: date, ends_on: date)
             .where.not(kind: HrAbsence::NON_REDUCING_KINDS)
  ...
  def effective_weekly_target_minutes(on_date: nil)
    ...
    if on_date && allows_monthly_plan?
      plan = HrMonthlyPlan.for_user(user).for_period(on_date.year, on_date.month).first
  ...
  def active_lecture_period(date)
    ...
    HrLecturePeriod.for_user(user).covering(date).first
  rescue ActiveRecord::StatementInvalid, NameError
    nil
  end
  ...
  def blocked_day?(date)
    ...
    HrAbsence.for_user(user).active.blocking
  ...
  def planning_kinds
    kinds = []
    kinds << HrAbsence::KIND_SCHOOL if effective_weekly_school_days.to_i.positive? || effective_school_weekdays.any?
    kinds << HrAbsence::KIND_BLOCK  if allows_monthly_plan?
    kinds = [HrAbsence::KIND_BLOCK, HrAbsence::KIND_SCHOOL] if kinds.empty? && planning_eligible?
    ...
    kinds << HrAbsence::KIND_WORKDAY if allows_monthly_plan?
    kinds
  end
  ...
  def monthly_plan_for(date)
    HrMonthlyPlan.for_user(user).for_period(date.year, date.month).first
  end
  ...
  def template_value(attr)
    hr_employment_type&.read_attribute(attr)
  end
