class HmLecturePeriod < ActiveRecord::Base
  ...
  def self.label_for(kind)
    case kind
    when KIND_LECTURE then I18n.t(:label_hm_lecture_kind_lecture)
    when KIND_BREAK   then I18n.t(:label_hm_lecture_kind_break)
    else kind.to_s
    end
  end
  ...
  def no_overlap
    ...
    errors.add(:base, :hm_lecture_period_overlap) if rel.exists?
  end
end
