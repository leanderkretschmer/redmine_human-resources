class HrAbsenceMailer < Mailer
  ...
  def absence_requested(user, recipients, absence_id)
    @absence   = HrAbsence.find(absence_id)
    @user      = @absence.user
    ...
    mail to: recipients,
         subject: I18n.t(:mail_subject_hr_absence_requested,
                         user: @user.name,
                         kind: HrAbsence.kind_label(@absence.kind))
  end
  ...
  def absence_decided(user, absence_id)
    ...
    mail to: @user.mail,
         subject: I18n.t(:mail_subject_hr_absence_decided,
                         status: HrAbsence.status_label(@absence.status),
                         kind:   HrAbsence.kind_label(@absence.kind))
  end
  ...
  def absence_edited(user, absence_id, editor_id)
    ...
    return if @user.mail.blank?
    mail to: @user.mail,
         subject: I18n.t(:mail_subject_hr_absence_edited,
                         editor: @editor.name,
                         kind:   HrAbsence.kind_label(@absence.kind))
  end
