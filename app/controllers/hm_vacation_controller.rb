class HmVacationController < ApplicationController
  ...
  def show
    load_state
    @new_absence = HmAbsence.new(kind: HmAbsence::KIND_VACATION,
  ...
  def load_state
    ...
    @absences = HmAbsence.for_user(User.current).vacation.order(starts_on: :desc).limit(50).to_a
    range_from = @month
