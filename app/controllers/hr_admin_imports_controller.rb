...
class HrAdminImportsController < ApplicationController
  ...
  def template
    ...
    send_data csv, filename: 'hr_timeclock_import_template.csv', type: 'text/csv; charset=utf-8'
  end

  def preview
    file = params[:file]
    if file.blank?
      flash.now[:error] = l(:notice_hr_import_no_file)
      return render :new
    end

    @rows = parse_csv(file.read)
    if @rows.empty?
      flash.now[:error] = l(:notice_hr_import_empty)
      return render :new
    end
    @users = User.active.sorted.to_a
  rescue StandardError => e
    flash.now[:error] = l(:notice_hr_import_parse_error, message: e.message)
    render :new
  end
  ...
  def commit
    ...
    HrWorkEntry.transaction do
      rows.each_with_index do |row, idx|
        ...
        if started_at.nil? || ended_at.nil?
          errors << l(:notice_hr_import_row_invalid, row: idx + 1)
          skipped += 1
          next
        end

        entry = HrWorkEntry.create!(
          user_id:    user_id,
          started_at: started_at,
          ended_at:   ended_at,
          state:      HrWorkEntry::STATE_COMPLETED,
          notes:      [row[:notes].presence, l(:notice_hr_import_note_suffix)].compact.join(' · ')
        )
        brk = row[:break_minutes].to_i
        if brk.positive?
          HrBreakEntry.create!(hr_work_entry_id: entry.id,
                               started_at: started_at,
    ...
    flash[:notice] = l(:notice_hr_import_done, created: created, skipped: skipped)
    flash[:error]  = errors.join(' · ') if errors.any?
    redirect_to hr_admin_import_path
  rescue StandardError => e
    flash[:error] = l(:notice_hr_import_commit_error, message: e.message)
    redirect_to hr_admin_import_path
  end
