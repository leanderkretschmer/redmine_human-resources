class HrVacationController < ApplicationController
  ...
  def show
    load_state
    @new_absence = HrAbsence.new(kind: HrAbsence::KIND_VACATION,
  ...
  def load_state
    ...
    @absences = HrAbsence.for_user(User.current).vacation.order(starts_on: :desc).limit(50).to_a
    range_from = @month
  ...
  def absence_params
    params.require(:hr_absence).permit(:starts_on, :ends_on, :reason, :first_day_half, :last_day_half)
  end
