class HmHomeofficeController < ApplicationController
  ...
  def show
    load_state
    @new_absence = HmAbsence.new(kind: HmAbsence::KIND_HOMEOFFICE,
  ...
  def create
    @new_absence = HmAbsence.new(absence_params.merge(
      kind: HmAbsence::KIND_HOMEOFFICE,
      user_id: User.current.id,
  ...
  def load_state
    ...
    @absences = HmAbsence.for_user(User.current).homeoffice.order(starts_on: :desc).limit(50).to_a
    range_from = @month
    ...
    @remaining = HmAbsence.homeoffice_remaining(User.current)
  end
